<x-guest-layout>
    <form method="POST" action="{{ route('login') }}" class="w-full bg-white p-8 rounded-lg shadow-md">
        ...
        <div class="mb-4 relative">
            ...
            <div class="relative">
                <x-text-input id="password" class="mt-1 block w-full pr-10"
                                type="password"
                                name="password"
                ...
                <button type="button" class="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5" onclick="togglePasswordVisibility('password')">
                    <svg class="h-5 w-5 text-gray-500 show-password" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    <svg class="h-5 w-5 text-gray-500 hide-password hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
            <a href="{{ route('google.login') }}" class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-sky-100 transition duration-150 ease-in-out">
    ...
    <script>
        function togglePasswordVisibility(inputId) {
            ...
            const showIcon = input.nextElementSibling.querySelector('.show-password');
            const hideIcon = input.nextElementSibling.querySelector('.hide-password');
